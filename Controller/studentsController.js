...
>>>>>>> decf6e8eaa9a109aebfa9a3fa1cc89bcfb2986bf
    }
    ...
    if (parentsMobileNumber) {
      const existingParent = await Student.findOne({
        parentsMobileNumber,
        _id: { $ne: req.params.id },
      });
      if (existingParent) {
        return res
          .status(400)
          .json({ message: "Parents' mobile number already used." });
      }
    }

    // Assign file paths to corresponding fields if files are provided
    if (req.files.image) {
      updatedData.image = req.files["image"]
        ? req.files["image"][0].filename
        : undefined;
    }
    if (req.files.guardianId) {
      updatedData.guardianId = req.files["guardianId"]
        ? req.files["guardianId"][0].filename
        : undefined;
    }
    if (req.files.studentId) {
      updatedData.studentId = req.files["studentId"]
        ? req.files["studentId"][0].filename
        : undefined;
    }
